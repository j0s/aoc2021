...
class Game(object):
    ...
    def draw(self, number: int) -> bool:
        ...
        for ix, (row, col) in enumerate(zip(self.rows, self.columns), 1):
            if len(row) == 0 or len(col) == 0:
                print(f'game {self.board_no} won after drawing {number}, '
                      f'completing {"column" if len(col)==0 else "row"} {ix}! '
    ...
    for draw in draws:
        for game in games[:]:
            if game.draw(draw):
                games.remove(game)
                if len(games) == 0:
                    print(f'final score: {game.remaining_points()} * {draw} = '
                          f'{game.remaining_points() * draw}')
                    exit(0)
