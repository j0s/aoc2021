...
def search_graph(graph: Dict[str, List[str]],
                 from_node: str,
                 visited: Set[str] = set()) -> List[List[str]]:
    paths = []
    visited.add(from_node)
    for neighbor in graph[from_node]:
        if (neighbor.islower() and neighbor in visited) or neighbor == 'start':
            continue
        elif neighbor == 'end':
            paths.append([from_node, 'end'])
        else:
            for path in search_graph(graph, neighbor, visited.copy()):
